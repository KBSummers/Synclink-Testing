...
static const char slusb_driver_version[] = "5.22";
...
static int auto_outputs = SerialSignal_DTR | SerialSignal_RTS;
module_param(debug_level, int, 0);
module_param(maxframe, int, 0);
module_param(auto_outputs, int, 0);
...
MODULE_PARM_DESC(auto_outputs, "Outputs set on open/CBAUD (RTS=0x20,DTR=0x80)");
...
#define SLUSB_CTS_FLOW   (1 << 0)
#define SLUSB_CHECK_CD   (1 << 1)
...
#else
static void slusb_port_remove(struct usb_serial_port *port);
#endif
...
static write_room_int_t slusb_write_room(struct tty_struct *tty);
static chars_in_buffer_int_t slusb_chars_in_buffer(struct tty_struct *tty);
...
static int  slusb_ioctl(struct tty_struct *tty,
			unsigned int cmd, unsigned long arg);
static int slusb_tty_tiocmiwait(struct tty_struct *tty, unsigned long arg);
static int slusb_get_icount(struct tty_struct *tty,
...
static struct usb_serial_driver slusb_serial_driver = {
	...
	.tiocmiwait = slusb_tty_tiocmiwait,
	.get_icount = slusb_get_icount
#endif
};
...
#else
static void slusb_tx_timeout(struct timer_list *t);
#endif
...
static bool slusb_hardware_init(struct slusb_info *info)
{
	...
	for (i=0 ; i < 10 ; i++) {
		rc = usb_bulk_msg(dev, info->in_pipe, info->poll_buf,
				  POLL_BUFFER_SIZE, &count, 500);
		if (rc)
			break;
		if (count > 1)
...
#else
	timer_setup(&info->tx_timer, slusb_tx_timeout, 0);
#endif
...
#if SYNCLINK_GENERIC_HDLC
	if (info->net_open) {
		// unregister_hdlc_device() bug checks if interface is up.
		// Workaround by forcing interface down.
		netif_stop_queue(info->netdev);
		hdlc_close(info->netdev);
...
static void slusb_change_params(struct slusb_info *info)
{
	...
	if (cflag & CBAUD) {
		if (auto_outputs & SerialSignal_DTR) {
			info->signals |= SerialSignal_DTR;
		}
		if (auto_outputs & SerialSignal_RTS) {
	...
	if (cflag & CRTSCTS)
		info->flags |= SLUSB_CTS_FLOW;
	else
		info->flags &= ~SLUSB_CTS_FLOW;

	if (cflag & CLOCAL)
		info->flags &= ~SLUSB_CHECK_CD;
	else
		info->flags |= SLUSB_CHECK_CD;
		       ...
		       const unsigned char *buf, int count)
{
	...
	else if (count && !tty_stopped(tty) && !tty->hw_stopped) {
...
static chars_in_buffer_int_t slusb_chars_in_buffer(struct tty_struct *tty)
{
	...
	unsigned long flags;
	u32 tfcr;
	chars_in_buffer_int_t fifo_count;
	...
	// transmit FIFO count contained in
	// TFCR bits 28:16 (13 bits = 8192 byte FIFO)
	// note: slusb_inl sleeps until TFCR read completes
	tfcr = slusb_inl(info, TFCR);
	fifo_count = (int)(0x1fff & (tfcr >> 16));
	count += fifo_count;

	if ((count == 0) && (info->tx_active != 0)) {
		// add one byte for shift register
		count++;
	}
			      ...
			      struct ktermios *old_termios)
{
	...
	if (!(old_termios->c_cflag & CBAUD) &&
	    tty_cflags(tty) & CBAUD) {
		if (auto_outputs & TIOCM_DTR) {
			info->signals |= SerialSignal_DTR;
		}
		if (!(tty_cflags(tty) & CRTSCTS) ||
		    !test_bit(TTY_THROTTLED, &tty->flags)) {
			if (auto_outputs & TIOCM_RTS) {
				info->signals |= SerialSignal_RTS;
			}
		}
		spin_lock_irqsave(&info->lock,flags);
...
// TIOCMIWAIT ioctl usb_serial wrapper
static int slusb_tty_tiocmiwait(struct tty_struct *tty, unsigned long arg)
{
	struct usb_serial_port *port = tty->driver_data;
	struct slusb_info      *info = usb_get_serial_port_data(port);
	return slusb_tiocmiwait(info, (int)arg);
}

// TIOCGICOUNT ioctl
static int slusb_tiocgicount(struct slusb_info *info, unsigned long arg)
{
	unsigned long flags;
...
// TIOCGICOUNT ioctl usb_serial wrapper
static int slusb_get_icount(struct tty_struct *tty,
	struct serial_icounter_struct *icount)
{
	struct usb_serial_port *port = tty->driver_data;
	struct slusb_info *info = usb_get_serial_port_data(port);
	return slusb_tiocgicount(info, (unsigned long)icount);
}
...
static int slusb_set_xctrl(struct slusb_info *info, int xctrl)
{
	...
	spin_lock_irqsave(&info->lock, flags);
	// apply input, preserving unused and Sipex bits
	info->xctrl = (info->xctrl & 0xfff00000) | (xctrl & 0x000fffff);
	slusb_outl(info, XCR, info->xctrl);
	spin_unlock_irqrestore(&info->lock, flags);
...
static int slusb_set_interface(struct slusb_info *info, int if_mode)
{
	...
	/*
	 * set Sipex serial interface options in XCR register
	 *
	 * XCR
	 * [31:25]  reserved, must be zero (preserve value)
	 *
	 ...
	 *
	 * [19:00]  xsync options (preserve value)
	 */
	info->xctrl = info->xctrl & 0xfe0fffff;
	switch(info->if_mode & MGSL_INTERFACE_MASK) {
	case MGSL_INTERFACE_RS232:
		info->xctrl |= SIPEX_RS232;
		break;
	case MGSL_INTERFACE_V35:
		info->xctrl |= SIPEX_V35;
		break;
	case MGSL_INTERFACE_RS422:
		info->xctrl |= SIPEX_EIA530;
		break;
	case MGSL_INTERFACE_RS530A:
		info->xctrl |= SIPEX_EIA530A;
		break;
	default:
		info->xctrl |= SIPEX_DISABLED;
		break;
	}
	if (!(info->if_mode & MGSL_INTERFACE_TERM_OFF))
		info->xctrl |= SIPEX_TERM;
	slusb_outl_queue(info, XCR, info->xctrl);
...
#endif
{
	...
	case MGSL_IOCWAITGPIO:
		...
		break;
	case TIOCGICOUNT:
		ret = slusb_tiocgicount(info, arg);
		break;
	case MGSL_IOCGXSYNC:
...
static void slusb_tx_eom(struct slusb_info *info, unsigned short status)
{
	...
	tty = tty_port_tty_get(&info->port->port);
	if (tty) {
		if (tty_stopped(tty) || tty->hw_stopped) {
...
static void slusb_tx_timeout(struct timer_list *t)
{
	struct slusb_info *info = from_timer(info, t, tx_timer);
#endif
	unsigned long flags;
...
#if LINUX_VERSION_CODE < VERSION(4,12,0)
...
static int __init slusb_init(void)
{
	...
	printk(KERN_INFO"%s %s loaded\n",
	       slusb_driver_name, slusb_driver_version);
	return 0;
}
